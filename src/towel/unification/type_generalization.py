...
from .type_bindings import TypeKind, TypeParameter, TypeTerm, contains_type_parameter
...
def _parameter_identities(term: TypeTerm) -> frozenset[str]:
    own = frozenset((term.parameter.identity,)) if term.parameter is not None else frozenset()
    return own.union(*(_parameter_identities(child) for child in term.children))
...
class _Builder:
    ...
    def merge(self, terms: tuple[TypeTerm, ...]) -> TypeTerm:
        ...
        if all(term == first for term in terms) and not contains_type_parameter(first):
            return first
        same_shape = all(
            term.kind is first.kind and len(term.children) == len(first.children) for term in terms
        )
...
def generalize_signatures(
    rows: Sequence[Sequence[TypeTerm]], reserved_names: Set[str]
    ...
    promise cannot be manufactured from unrelated call sites.
    """
    if not rows or len(rows[0]) < 2 or any(len(row) != len(rows[0]) for row in rows):
        return ()
    builder = _Builder(reserved_names)
    try:
    ...
    if not parameters:
        return ()
    input_binders = frozenset().union(*(_parameter_identities(term) for term in types[:-1]))
