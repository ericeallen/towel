...
# Round-3 audit case mod_three_modules_cluster (modules: three-sites-three-modules), whose
# behaviour the audit found kept.
if __name__ == "__main__":
    import pkg.a as pkg_a
    import pkg.b as pkg_b
    import pkg.c as pkg_c
    import pkg
    import sys
    import copy
    import re
    ...
    def _probe_4():
        import subprocess, sys
        r = subprocess.run([sys.executable, '-c', 'import pkg.b, sys; print(sorted(k for k in sys.modules if k.startswith(("pkg", "other", "tests"))))'], capture_output=True, text=True)
        __result__ = (r.returncode, r.stdout, r.stderr.strip().splitlines()[-1:] )
        return __result__
    _value('probe 4', _probe_4)
    _calls('pkg_c.fc', pkg_c.fc, [([1, 2],)])
    def _probe_6():
        import subprocess, sys
        r = subprocess.run([sys.executable, '-c', 'import pkg.c, sys; print(sorted(k for k in sys.modules if k.startswith(("pkg", "other", "tests"))))'], capture_output=True, text=True)
        __result__ = (r.returncode, r.stdout, r.stderr.strip().splitlines()[-1:] )
        return __result__
    _value('probe 6', _probe_6)
