...
* **Definite assignment.** ``definitely_bound_after`` agrees with a reference
  that enumerates every fall-through path abstractly. The grammar has no
  ``del`` and no ``except ... as name``, which the analysis over-approximates
  path-insensitively, so the reference stays a plain path enumeration.
...
All three are bounded (``max_examples=50``), deterministic
(``derandomize=True``), and have no deadline, so they cannot flake on a slow
machine.
"""
...
Flow = Union[Bind, Leave, Branch, Loop, Try]
FLOW_NAMES = ("a", "b", "c")
