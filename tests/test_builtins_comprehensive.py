...
import unittest
from src.towel.unification.builtins import (
    PYTHON_BUILTINS,
    is_builtin,
    filter_builtins
)
...
class TestPythonBuiltinsConstant(unittest.TestCase):
    ...
    def test_contains_common_functions(self):
        ...
        for builtin in common_builtins:
            self.assertIn(builtin, PYTHON_BUILTINS,
                         f"Common builtin '{builtin}' should be in PYTHON_BUILTINS")
    ...
    def test_contains_common_constants(self):
        ...
        for const in constants:
            self.assertIn(const, PYTHON_BUILTINS,
                         f"Builtin constant '{const}' should be in PYTHON_BUILTINS")
    ...
    def test_contains_common_exceptions(self):
        ...
        for exc in exceptions:
            self.assertIn(exc, PYTHON_BUILTINS,
                         f"Builtin exception '{exc}' should be in PYTHON_BUILTINS")

    def test_not_empty(self):
        """Test that PYTHON_BUILTINS is not empty."""
        self.assertGreater(len(PYTHON_BUILTINS), 0,
                          "PYTHON_BUILTINS should not be empty")

    def test_all_strings(self):
        """Test that all entries are strings."""
        for item in PYTHON_BUILTINS:
            self.assertIsInstance(item, str,
                                f"All builtin names should be strings, found {type(item)}")
...
class TestIsBuiltin(unittest.TestCase):
    ...
    def test_common_builtins_return_true(self):
        ...
        for builtin in common_builtins:
            self.assertTrue(is_builtin(builtin),
                          f"is_builtin('{builtin}') should return True")
    ...
    def test_constants_return_true(self):
        ...
        for const in constants:
            self.assertTrue(is_builtin(const),
                          f"is_builtin('{const}') should return True")
    ...
    def test_exceptions_return_true(self):
        ...
        for exc in exceptions:
            self.assertTrue(is_builtin(exc),
                          f"is_builtin('{exc}') should return True")

    def test_user_names_return_false(self):
        """Test that user-defined names return False."""
        user_names = [
            'my_function', 'user_var', 'CustomClass', 'foo', 'bar',
            'calculate_result', 'process_data', 'x', 'y', 'z'
        ]

        for name in user_names:
            self.assertFalse(is_builtin(name),
                           f"is_builtin('{name}') should return False")
    ...
    def test_stdlib_module_names_return_false(self):
        ...
        stdlib_modules = [
            'os', 'sys', 'math', 'json', 'random', 'datetime',
            'collections', 'itertools', 'functools'
        ]

        for module in stdlib_modules:
            self.assertFalse(is_builtin(module),
                           f"is_builtin('{module}') should return False (it's a module)")
...
class TestFilterBuiltins(unittest.TestCase):
    ...
    def test_removes_all_builtins(self):
        ...
        self.assertEqual(result, {'my_var', 'user_function'},
    ...
    def test_no_builtins_returns_same(self):
        ...
        self.assertEqual(result, user_names,
                        "Set with no builtins should be unchanged")
    ...
    def test_preserves_user_names_exactly(self):
        ...
        self.assertEqual(result, names,
                        "Case-different names should be preserved")

    def test_mixed_names(self):
        """Test filtering mixed set of builtins and user names."""
        names = {
            'x', 'y', 'print', 'len', 'result', 'ValueError',
            'calculate', 'Exception', 'data', 'str'
        }
        ...
        self.assertEqual(result, expected,
                        "Should filter out builtins correctly")
    ...
    def test_returns_new_set(self):
        ...
        self.assertEqual(names, {'x', 'print', 'y'},
    ...
    def test_filters_constants(self):
        ...
        self.assertEqual(result, {'x', 'result'},
                        "Should filter out builtin constants")
    ...
    def test_filters_exceptions(self):
        ...
        self.assertEqual(result, {'error_handler', 'result'},
                        "Should filter out builtin exceptions")
...
class TestSpecificBuiltins(unittest.TestCase):
    ...
    def test_introspection_builtins(self):
        ...
        intro_builtins = [
            ...
            'setattr', 'delattr', 'dir', 'vars', 'id', 'callable'
        ]

        for builtin in intro_builtins:
            self.assertTrue(is_builtin(builtin),
                          f"Introspection builtin '{builtin}' should be recognized")
    ...
    def test_io_builtins(self):
        ...
        for builtin in io_builtins:
            self.assertTrue(is_builtin(builtin),
                          f"I/O builtin '{builtin}' should be recognized")
    ...
    def test_math_builtins(self):
        ...
        for builtin in math_builtins:
            self.assertTrue(is_builtin(builtin),
                          f"Math builtin '{builtin}' should be recognized")

    def test_debugging_builtins(self):
        """Test that debugging builtins are recognized."""
        self.assertTrue(is_builtin('breakpoint'),
                       "breakpoint should be recognized as a builtin")
...
class TestIntegration(unittest.TestCase):
    ...
    def test_realistic_variable_filtering(self):
        ...
        variables = {
            'data', 'result', 'x', 'y', 'i', 'j',  # user variables
            'print', 'len', 'range', 'str',  # builtins used
            'ValueError', 'TypeError',  # exceptions caught
            'process', 'calculate', 'transform'  # user functions
        }
        ...
        self.assertEqual(filtered, expected,
                        "Should correctly filter realistic variable set")

    def test_preserves_similar_but_different_names(self):
        """Test that names similar to builtins are preserved."""
        names = {
            'my_print', 'print_result', 'string', 'integer',
            'list_items', 'dictionary', 'maximum', 'minimum'
        }
        ...
        self.assertEqual(filtered, names,
                        "Names similar to builtins should be preserved")
