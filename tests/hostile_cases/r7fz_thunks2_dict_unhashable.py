...
# Round-3 audit case th2_dict_unhashable (thunks2: dict-display-raises-before-thunk). P1-3: a dict
# display that raises TypeError for an unhashable key precedes a thunk the helper evaluates; the
# thunk is passed eagerly and its effect now comes first.
def tr(tag, v=None):
    print("tr", tag)
    return v
...
if __name__ == "__main__":
    ...
    _calls('pkg_m.f1', pkg_m.f1, [('k', pkg_m.Loud(3)), ([], pkg_m.Loud(3))])
    _calls('pkg_m.f2', pkg_m.f2, [('k', pkg_m.Loud(3)), ([], pkg_m.Loud(3))])
