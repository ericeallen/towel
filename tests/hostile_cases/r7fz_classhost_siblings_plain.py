...
# Round-3 audit case cls_siblings_plain (classhost: siblings-module-helper), whose behaviour the
# audit found kept.
class Base:
    def __init__(self):
        self.base = 1
...
if __name__ == "__main__":
    ...
    _value('pkg_m.B().m2(1)', lambda: pkg_m.B().m2(1))
    _value('sorted(vars(pkg_m.A))', lambda: sorted(vars(pkg_m.A)))
    _value('sorted(vars(pkg_m.Base))', lambda: sorted(vars(pkg_m.Base)))
