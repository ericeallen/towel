...
# Round-3 audit case misc_binder_renamed_unbound_message_inner (misc: renamed-binder-read-inside-
# block), whose behaviour the audit found kept.
def f1(xs, flag):
    if flag:
        last = len(xs)
    doubled = last * 2
...
if __name__ == "__main__":
    ...
    _calls('pkg_m.f1', pkg_m.f1, [([1], True), ([], False)])
    _calls('pkg_m.f2', pkg_m.f2, [([1], True), ([], False)])
