...
# Round-3 audit case pre_for_else_prebound (prebound: for-else-target). P1-1: as
# pre_for_read_in_block, with a for-else: the name bound before the loop is read after it.
def f1(xs):
    i = -1
    for i in xs:
        if i > 5:
...
if __name__ == "__main__":
    ...
    _calls('pkg_m.f1', pkg_m.f1, [([1, 7],), ([],), ([1, 2],)])
    _calls('pkg_m.f2', pkg_m.f2, [([1, 7],), ([],), ([1, 2],)])
