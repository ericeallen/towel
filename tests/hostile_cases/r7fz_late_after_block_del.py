...
# Round-3 audit case late_after_block_del (late: del-after-block-not-a-read). P1-8: a later del
# count is the only use of count, which the block binds; it is not counted as a read, so the
# helper does not return count.
def f1(xs, extra):
    count = len(xs)
    total = sum(xs)
