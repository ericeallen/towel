...
# Round-3 audit case cls_zero_arg_super (classhost: super), whose behaviour the audit found kept.
class Base:
    def val(self, n):
        print("Base.val", n)
        return n
...
if __name__ == "__main__":
    ...
    _value('pkg_m.A().m2(1)', lambda: pkg_m.A().m2(1))
    _value('pkg_m.C().m1(1)', lambda: pkg_m.C().m1(1))
    _value('pkg_m.C().m2(1)', lambda: pkg_m.C().m2(1))
    _value('pkg_m.A.m1(pkg_m.C(), 3)', lambda: pkg_m.A.m1(pkg_m.C(), 3))
