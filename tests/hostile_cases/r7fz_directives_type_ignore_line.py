# Round-3 audit case dir_type_ignore_line (directives: type-ignore-in-block), whose behaviour the
# audit found kept.
from typing import Any


class Obj:
    def __init__(self) -> None:
        setattr(self, "missing", 5)
        setattr(self, "other", 7)


def f1(o: Obj, n: int) -> int:
    t = n + 1
    v = o.missing  # type: ignore[attr-defined]
    w = t * 2 + v
    print("f1", t, v, w)
    return w


def f2(o: Obj, n: int) -> int:
    t = n + 2
    v = o.missing  # type: ignore[attr-defined]
    w = t * 2 + v
    print("f2", t, v, w)
    return w


if __name__ == "__main__":
    import sys

    import copy
    import re

    def _shown(value):
        return re.sub(r"0x[0-9a-fA-F]+", "0xADDR", repr(value))

    def _calls(label, function, argument_sets):
        for arguments in argument_sets:
            arguments = copy.deepcopy(arguments)
            try:
                outcome = "-> " + _shown(function(*arguments))
            except Exception as error:
                outcome = f"raised {type(error).__name__}: {_shown(str(error))}"
            print(label, outcome, "| arguments after:", _shown(arguments))

    def _value(label, produce):
        try:
            print(label, "=", _shown(produce()))
        except Exception as error:
            print(label, "raised", type(error).__name__, _shown(str(error)))

    pkg_m = sys.modules[__name__]
    _calls('pkg_m.f1', pkg_m.f1, [(pkg_m.Obj(), 1)])
    _calls('pkg_m.f2', pkg_m.f2, [(pkg_m.Obj(), 1)])
