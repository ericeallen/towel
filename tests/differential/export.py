...
def fixture_name(outcome: Outcome, battery: Battery, prefix: str) -> str:
    """The fixture's name: ``r<prefix>_grammar_u0898`` (``r<prefix>_scope_u0042``) or ``xf<prefix>_...``.
    ...
    if SCOPE_FAMILY in outcome.case.features:
        family, stem = SCOPE_FAMILY, outcome.case.name.removeprefix(f"{SCOPE_FAMILY}_")
    else:
        family, stem = "grammar", outcome.case.name.removeprefix("gram_")
    return f"{'r' if battery == 'hostile_cases' else 'xf'}{prefix}_{family}_{stem}{suffix}"
...
def _header(outcome: Outcome, comment: str) -> str:
    ...
    if SCOPE_FAMILY in case.features:
        generator, how = "scope_grammar", how + f" --family {SCOPE_FAMILY}"
    lines = [
        f"Generated by tests.differential.{generator} from seed {case.seed}"
