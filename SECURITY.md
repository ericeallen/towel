...
Towel's own analysis parses Python syntax without executing the analyzed project. The type checkers it drives when `--types` is on (the default) are the exception to keep in mind: mypy runs in-process with Towel's own options and loads no plugin from the project; pyright runs as a subprocess that reads the project's pyright configuration but is given Towel's interpreter (`--pythonpath`), so a `venv` setting there is never executed, and is started with `python -P`, so a project package named like a standard module (sphinx's `locale`) is never imported from the directory it runs in. The `pyright` PyPI package downloads a Node runtime on first use. Pass `--no-types` when analyzing code you do not trust. The behavioral test harness executes fixtures and must be used only on trusted code. The rename assistant prints a prompt containing source excerpts for the user to transfer manually; consider the destination before sharing confidential code.
