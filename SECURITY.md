...
Security fixes are provided for the latest released version, currently **1.772**. Earlier versions are not supported; the PyPI releases `1.0.0`–`1.0.4` are yanked for broken import handling and are not a recommended installation target. Upgrade to the latest release rather than relying on a fix to an older one.
...
Towel's own analysis parses Python syntax without executing the analyzed project. Optional type checkers read project source and configuration: mypy runs in an owned worker with the project's checking rules, each build in a forked child of it that exits once it has answered, while project plugins, configured executables and report destinations are disabled; pyright runs as a long-lived `pyright-langserver` subprocess per project, started with Towel's interpreter, or as a command-line subprocess with Towel's interpreter (`--pythonpath`), so a project `venv` setting does not select an executable. Python module launches for mypy, pyright and Ruff use isolated mode (`-I`), and child tool environments exclude inherited Python startup/import settings, including for PATH executable fallbacks. Whole-project pyright verification uses a private copy of the project's sources, stubs and checker configuration, which is made once per run, kept in step with the project while the run lasts by comparing content, and removed when the oracle is closed. A typed run that used a language server starts one further pyright command-line subprocess at the end to confirm the finished project. Checker failure is distinct from a successful check and cannot certify a generated change. The `pyright` PyPI package downloads a Node runtime on first use. Pass `--no-types` when analyzing code you do not trust. The behavioral test harness executes fixtures and must be used only on trusted code. The rename assistant prints a prompt containing source excerpts for the user to transfer manually; consider the destination before sharing confidential code.
