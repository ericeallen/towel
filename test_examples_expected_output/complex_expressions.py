...
def calculate_discount_tier1(price, customer_level):
    ...
    return __extracted_func_88(5, 100, 10, 50, customer_level, price)


def calculate_discount_tier2(price, customer_level):
    """Tier 2: Different boolean expression, same structure."""
    return __extracted_func_88(3, 200, 8, 75, customer_level, price)


def transform_data_format_a(data, processor):
    """Format A: Nested method calls and indexing."""
    return __extracted_func_94('values', lambda *args, **kwargs: processor.normalize(data['values']).upper(*args, **kwargs), data, processor)


def transform_data_format_b(data, processor):
    """Format B: Different nested calls, same pattern."""
    return __extracted_func_94('items', lambda *args, **kwargs: processor.normalize(data['items']).lower(*args, **kwargs), data, processor)
...
def __extracted_func_88(__param_0, __param_1, __param_2, __param_3, customer_level, price):
    base = price * 0.9
...
def __extracted_func_94(__param_0, __param_1, data, processor):
