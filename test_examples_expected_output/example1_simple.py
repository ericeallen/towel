...
def __extracted_func_78(__param_87):
    if not __param_87.get('id'):
        raise ValueError('User ID is required')
    if not __param_87.get('name'):
        raise ValueError('User name is required')
    if len(__param_87.get('name', '')) < 2:
        raise ValueError('User name too short')
