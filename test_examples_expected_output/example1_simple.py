...
def __extracted_func_46(__param_40):
    if not __param_40.get('id'):
        raise ValueError('User ID is required')
    if not __param_40.get('name'):
        raise ValueError('User name is required')
    if len(__param_40.get('name', '')) < 2:
        raise ValueError('User name too short')
