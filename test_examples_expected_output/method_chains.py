...
def process_api_response_v1(response, validator, transformer):
    ...
    return __extracted_func_1139('data', 'items', response, transformer, validator)
...
def process_api_response_v2(response, validator, transformer):
    ...
    return __extracted_func_1139('payload', 'records', response, transformer, validator)


def update_model_fields_a(model, updates, validator):
    """Version A: Fluent interface pattern."""
    # Fluent method calls
    return __extracted_func_1135(model.set_name, 'name', lambda *args, **kwargs: model.set_name(updates['name']).set_description(*args, **kwargs), 'description', model, updates, validator)


def update_model_fields_b(model, updates, validator):
    """Version B: Different keys, same fluent pattern."""
    # Same fluent pattern
    return __extracted_func_1135(model.set_title, 'title', lambda *args, **kwargs: model.set_title(updates['title']).set_content(*args, **kwargs), 'content', model, updates, validator)


def query_database_v1(db, filters, mapper):
    """Version 1: Database query builder pattern."""
    # Method chaining query
    return __extracted_func_971(18, db, mapper)
...
def query_database_v2(db, filters, mapper):
    ...
    return __extracted_func_971(21, db, mapper)


def process_stream_v1(stream, parser, handler):
    """Version 1: Stream processing with method calls."""
    # Stream operations
    return __extracted_func_1022(1000, handler, parser, stream)
...
def transform_entity_v2(entity, transformer, validator):
    ...
    return __extracted_func_1087('completed', entity, transformer, validator)
...
def aggregate_results_a(results, aggregator, formatter):
    ...
    return __extracted_func_1147('total', 'average', 'maximum', average, formatter, maximum, total)
...
def aggregate_results_b(results, aggregator, formatter):
    ...
    return __extracted_func_1147('sum', 'mean', 'max', average, formatter, maximum, total)


def __extracted_func_971(__param_0, db, mapper):
...
def __extracted_func_1022(__param_0, handler, parser, stream):
...
def __extracted_func_1087(__param_0, entity, transformer, validator):
...
def __extracted_func_1108(__param_0, cache, data, serializer):
...
def __extracted_func_1135(__param_0, __param_1, __param_2, __param_3, model, updates, validator):
...
def __extracted_func_1139(__param_0, __param_1, response, transformer, validator):
...
def __extracted_func_1147(__param_0, __param_1, __param_2, average, formatter, maximum, total):
