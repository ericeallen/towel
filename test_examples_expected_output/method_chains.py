...
def query_database_v1(db, filters, mapper):
    ...
    return __extracted_func_176(18, db, mapper)
...
def query_database_v2(db, filters, mapper):
    ...
    return __extracted_func_176(21, db, mapper)
...
def __extracted_func_176(__param_215, db, mapper):
    results = db.table('users').where('age', '>', __param_215).where('status', '=', 'active').order_by('created_at', 'desc').limit(100).get()
