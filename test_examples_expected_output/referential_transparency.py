...
def update_mutable_state_v1(items, counter):
    ...
    for item in items:
        ...
        __extracted_func_2101(item, counter)
...
def update_mutable_state_v2(items, counter):
    ...
    for item in items:
        ...
        __extracted_func_2101(item * 2, counter)
...
def accumulate_with_closure_v1(items, accumulator):
    ...
    return __extracted_func_2065(2, accumulator, items)
...
def accumulate_with_closure_v2(items, accumulator):
    ...
    return __extracted_func_2065(3, accumulator, items)
...
def transform_with_early_return_a(data, validator):
    ...
    for item in data:
        ...
        cleaned = item.strip()
        return __extracted_func_2098(lambda *args, **kwargs: cleaned.upper(*args, **kwargs), cleaned, validator)
...
def transform_with_early_return_b(data, validator):
    ...
    for item in data:
        ...
        cleaned = item.strip()
        return __extracted_func_2098(lambda *args, **kwargs: cleaned.lower(*args, **kwargs), cleaned, validator)
...
def nested_scope_capture_v1(outer_data, processor):
    ...
    """
    return __extracted_func_2011(outer_data, processor)
...
def nested_scope_capture_valid_v2(outer_data, processor):
    ...
    """
    return __extracted_func_2047(200, outer_data, processor)


def modify_external_state_a(data, cache, metrics):
    """Version A: Modifies multiple external objects."""
    return __extracted_func_2086(2, cache, data, metrics)


def modify_external_state_b(data, cache, metrics):
    """Version B: Different multiplier, same modification pattern."""
    return __extracted_func_2086(3, cache, data, metrics)


def __extracted_func_2011(outer_data, processor):
    return __extracted_func_2047(100, outer_data, processor)


def __extracted_func_2047(__param_0, outer_data, processor):
    outer_var = __param_0
...
def __extracted_func_2065(__param_0, accumulator, items):
...
def __extracted_func_2086(__param_0, cache, data, metrics):
...
def __extracted_func_2098(__param_0, cleaned, validator):
    if not cleaned:
        return None
...
def __extracted_func_2101(__param_0, counter):
