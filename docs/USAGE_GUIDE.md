...
| `excluded_directories` | `()` | Directory names left out of the program (`--exclude`), at any depth: nothing in them is refactored, and no check that reads the whole program reads them, the import model included. |
...
unsafe changes. `analyze_files` makes no such check first, but refuses the same
