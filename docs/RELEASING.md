...
The latest release is `1.618`, published to PyPI on September 17, 2026; `1.414` preceded it on September 15 (the first release after the production-readiness pass recorded in [PRODUCTION_READINESS.md](PRODUCTION_READINESS.md)). The next release is not yet numbered. It carries everything under `[Unreleased]` in [CHANGELOG.md](../CHANGELOG.md): reuse of an existing function for whole-body duplicates, formatting of generated code with the project's formatter, type annotations on helpers verified by the project's type checker, the `format` and `types` extras, the paired command-line options, the exact incremental global passes and the other performance work, and the fixes the 141-project ecosystem run found. Preparing artifacts does not authorize uploading them, changing repository visibility, creating remote tags, or contacting users. Publication is a separate maintainer decision.
