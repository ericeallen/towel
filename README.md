...
When verification is enabled, all prospective changed modules are checked together with unchanged consumers. Type errors trigger a retry with helper annotations replaced by `Any`, then with no helper annotations; each variant must pass. Checker failure or remaining errors decline the proposal. Without a checker, Towel copies and does not reason: unions are unreduced and disagreeing declarations leave the return bare. These unions do not preserve every relationship between arguments and results; [type-parameter inference](https://github.com/ericeallen/towel/blob/v1.732.post1/docs/proposals/type-parameters.md) is proposed for a later release. [The architecture document](https://github.com/ericeallen/towel/blob/v1.732.post1/docs/ARCHITECTURE.md#helper-annotations) gives the complete rules, including annotation placement and quoting.
