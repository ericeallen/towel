...
The pipeline parses modules, analyzes scopes, collects functions and classes, compares candidate blocks, and constructs extraction proposals. It supports same-file candidates and, with `--cross-module`, cross-file ones, parameter differences, return propagation, and selected class-method extractions. Directory mode skips hidden directories, `__pycache__`, `node_modules`, every environment packages are installed into (a directory holding a `pyvenv.cfg` or a `conda-meta`, whatever it is called, so a package of the project's own named `env` or `venv` is refactored like any other), the names given by `--exclude`, and symlinked files. Some checks read the whole program around the code being refactored, from the project root down, for what could make a change unsafe: a decorator applied by hand, a builtin a test patches into a module, a module whose asserts pytest rewrites. A file of the program that does not parse on the Python Towel runs on may still run on a newer one, doing what those checks never saw, so every mode refuses the run before writing anything, naming each such file and what the parser said. Run Towel on a Python that parses it (the refusal names the newest the project declares), or, if the file is not meant to run, such as deliberately invalid test data, leave it out with `--exclude` and its name or its directory's; the refusal names one that clears it.
...
`towel dry PKG PKG --exclude tests` leaves every directory or file of that name, at any depth, unchanged (repeatable, and accepted by `preview` too); use it for packages that carry their test suite inside themselves, for a file such as `--exclude benchmark.py`, and for a stray copy of a package that makes its imports ambiguous, since the program's import model reads nothing in an excluded directory. It takes a name, not a path or a pattern: for `tests/**/hooks`, pass `--exclude hooks`. The checks that read the whole program still read what it names, so an excluded test suite's `mock.patch` of a builtin still declines a change it would notice; only an excluded file that does not parse is taken for no part of the program. Large analyses fork worker processes after parsing when a timed probe projects enough work; set `TOWEL_WORKERS=1` to stay on one core or another value to cap the workers. Set `TOWEL_CHECK_AST_IMMUTABLE=1` to verify on every cache reuse that analysis left the module's AST untouched.
