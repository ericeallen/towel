...
**Disk.** Both in-place and out-of-place refactoring need temporary space for staged changes and recovery journals. Out-of-place refactoring also copies the whole project (its Python sources, stubs and configuration; the target in full) into a private temporary stage, removed when the run ends, and writes only the refactored target to the output directory. Optional type checking adds, for the life of the run, a second copy of the project's checker inputs for pyright and a mypy cache directory; both are removed when the oracle is closed. Recovery journals, `.towel-transaction-<id>` directories at the common root of a batch, hold the original source bytes until you resolve them; a pending journal blocks a later run only when its manifest names a file that run would change.
...
To roll back an interrupted batch, use `towel recover /path/to/.towel-transaction-<id>` (the name the run reported). Recovery refuses detected conflicting edits and keeps the journal for resolution. Review local journals before recovery; they contain original source bytes. Do not delete a journal before resolving the interrupted operation. An out-of-place run refactors the target inside a private copy of its whole project, exactly as an in-place run would, so import cycles and import-time effects through modules outside the target are seen; the output directory is written all at once only when the run succeeds, so a failed or interrupted run leaves no output behind. A project root holding more than 20,000 Python files is refused rather than copied; refactor a copy of it in place instead.
...
Towel annotates a helper only in code that already uses annotations, from what the call sites declare and what the project's type checker says. It uses mypy or Pyright according to the project's configuration; with both configured, mypy infers and both verify, the first to reject settling a candidate so the other is not asked. A checker the project configures but that is not installed where Towel runs refuses the typed run before anything is written, naming the checker: install it, or rerun with `--no-types` to refactor unverified. Only a project that configures no checker is checked by whichever one is installed, mypy first. The project's configured mypy plugins (django-stubs, pydantic, SQLAlchemy and the like) are loaded and run in every check, as in the project's own mypy run, so they execute project-configured code; a plugin that cannot be loaded where Towel runs refuses the typed run before anything is written, quoting mypy's error. A run checked through a language server is confirmed once more at the end by a pyright started from nothing. Before using the checker, Towel checks the original project. If it already has type errors, Towel aborts: fix those errors or explicitly rerun with `--no-types`. That option leaves existing source annotations intact and generates unannotated helpers. A checker crash or timeout is reported as a distinct checker failure.
